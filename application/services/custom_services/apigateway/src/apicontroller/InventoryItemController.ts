...
export class playerController implements Controller {
    ...
    private initializeRoutes() {
        this.router.delete('/player/:id', this.GpDelete);
this.router.get('/player/get/search', this.GpSearch);
this.router.put('/player/get/update', this.GpSearchForUpdate);
this.router.put('/player', this.GpUpdate);
this.router.get('/player/:id', this.GpGetNounById);
this.router.get('/player', this.GpGetAllValues);
this.router.post('/player', this.GpCreate);
    }

public GpDelete(req: Request, res: Response) {
            new CustomLogger().showLogger('info', 'Enter into playerController.ts: GpDelete');
        new ApiAdaptar().delete(Constant.playersmanagerURL + `${req.url}` )
...
public GpSearch(req: Request, res: Response) {
        ...
        new ApiAdaptar().get(Constant.playersmanagerURL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/desktop' ? res.send(result) : res.send(null)
...
public GpSearchForUpdate(req: Request, res: Response) {
        ...
        new ApiAdaptar().put(Constant.playersmanagerURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/desktop' ? res.send(result) : res.send(null)
...
public GpCreate(req: Request, res: Response) {
        ...
        new ApiAdaptar().post(Constant.playersmanagerURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/desktop' ? res.send(result) : res.send(null)
